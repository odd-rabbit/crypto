...
if __name__ == '__main__':
    ...
    # API().orderbook('BTC_USDT', 100).to_csv('book.csv', header=True)
    # API().trades('BTC_USDT', 100).to_csv('trade.csv', header=True)
    while True:
        time.sleep(1)
        # print(API().price('bitcoin'))
        API().tickers(['BTC_USDT']).to_csv('tickers.csv', mode='a')
